...
mysql -u root -e "CREATE DATABASE IF NOT EXISTS moodle" -ppassword
...
mkdir db
cd db
git clone https://github.com/abhinay100/moodle_db.git .



mysql -u root moodle < /tmp/db/moodle.sql -ppassword
