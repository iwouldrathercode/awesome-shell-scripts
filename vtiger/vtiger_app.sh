...
chmod 777 -R /var/www/
git clone https://github.com/abhinay100/vtiger_app.git .
chmod 777 -R /var/www/
...
mysql -u root -e "CREATE DATABASE IF NOT EXISTS vtiger" -ppassword
...
git clone https://github.com/abhinay100/vtiger_db.git .
mysql -u root vtiger < /tmp/db/vtiger.sql -ppassword
